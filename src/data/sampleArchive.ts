...
export const sampleArchiveImages: ArchiveImage[] = celebrityNames.flatMap((canonicalName, celebrityIndex) =>
  Array.from({ length: 50 }, (_, photoIndex) => {
    const imageNumber = celebrityIndex * 50 + photoIndex + 1;
    const year = 2023 + (photoIndex % 5);
    ...
    return {
      ...
      source_text: {
        title: photoIndex % 3 === 0 ? `${canonicalName} red carpet arrival` : null,
        caption: photoIndex % 3 === 0 ? `${canonicalName} photographed at the annual gala.` : null,
        alt_text: `${canonicalName} at the gala`,
      },
      celebrities: [
        {
          canonical_name: canonicalName,
          identification_source: photoIndex % 3 === 0 ? "AI_image recognition only + Meta" : "AI_image recognition only",
          status: "Approved",
          AIresponse: {
